...
const CATEGORIES = [
  ...
  { id: 'customer', emoji: '👤', label: 'Customer Issue' },
  { id: 'app', emoji: '📱', label: 'App Bug' },
  { id: 'route', emoji: '🗺️', label: 'Wrong Route' },
  { id: 'incentive', emoji: '🎁', label: 'Incentive Issue' },
  { id: 'account', emoji: '🔐', label: 'Account Issue' },
  { id: 'other', emoji: '❓', label: 'Other' },
];

const PAST_COMPLAINTS = [
  { id: 'DCP001', category: 'PAYMENT', title: 'Delivery bonus not credited', status: 'resolved', date: '15 Mar 2026' },
  { id: 'DCP002', category: 'APP', title: 'App crashed during delivery', status: 'in_review', date: '23 Mar 2026' },
];
...
export default function ComplaintScreen({ navigation }) {
  ...
  const handleSubmit = () => {
    ...
    if (description.trim().length < 20) { Alert.alert('More Details', 'Please describe your issue in at least 20 characters.'); return; }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        <BackButton navigation={navigation} fallback="Home" style={{ marginRight: 12 }} />
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>🆘 Help & Support</Text>
          <Text style={styles.headerSub}>Delivery Partner Support</Text>
        </View>
      </View>
      ...
      {tab === 'new' ? (
        <ScrollView contentContainerStyle={styles.form} showsVerticalScrollIndicator={false}>
          <View style={styles.urgentBox}>
            <Text style={styles.urgentText}>🚨 Emergency? Call support directly: <Text style={{ color: '#4CAF50', fontWeight: '800' }}>9876543000</Text></Text>
          </View>
          ...
          <TextInput style={styles.input} placeholder="Short title" placeholderTextColor="#C4C4C4" value={title} onChangeText={setTitle} maxLength={80} />
          <Text style={styles.sectionTitle}>📄 Details *</Text>
          <TextInput style={[styles.input, styles.textArea]} placeholder="Describe your issue in detail..." placeholderTextColor="#C4C4C4" value={description} onChangeText={setDescription} multiline numberOfLines={5} maxLength={500} />
        ...
        <ScrollView contentContainerStyle={styles.form} showsVerticalScrollIndicator={false}>
          ...
          {submitted && (
            <View style={styles.complaintCard}>
              <View style={styles.complaintTop}>
                <Text style={styles.complaintId}>#DCP00{PAST_COMPLAINTS.length + 1}</Text>
      ...
      <Modal visible={showSuccess} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalSub}>Ticket: #DCP00{PAST_COMPLAINTS.length + 1}</Text>
            <Text style={styles.modalMsg}>We'll respond within 24 hours. For urgent issues call 9876543000.</Text>
...
const styles = StyleSheet.create({
  ...
  urgentBox: { backgroundColor: '#FFF3F3', borderRadius: 14, padding: 14, borderLeftWidth: 4, borderLeftColor: '#EF4444', marginBottom: 16 },
  urgentText: { color: '#555', fontSize: 13, lineHeight: 20 },
  ...
  submitBtn: { backgroundColor: '#4CAF50', borderRadius: 16, padding: 18, alignItems: 'center', marginBottom: 16, shadowColor: '#4CAF50', shadowOpacity: 0.4, shadowRadius: 12, elevation: 8 },
  ...
  modalSub: { fontSize: 14, color: '#4CAF50', fontWeight: '700', marginBottom: 12 },
  modalMsg: { fontSize: 14, color: '#555', textAlign: 'center', lineHeight: 22, marginBottom: 20 },
  modalBtn: { backgroundColor: '#4CAF50', borderRadius: 14, padding: 14, width: '100%', alignItems: 'center', marginBottom: 10 },
